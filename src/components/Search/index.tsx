  ...
  useEffect(() => {
    ...
    async function getEligibleAccounts(groupName: string) {
      try {
        setLoading(true);
        setError("");
        const latestGroups = await axios.get(`${env.sismoHubUrl}/groups/latests`);
      ...
      <Container>
        ...
        <SearchInputWrapper>
          ...
          <IconWrapper>
            <MagnifyingGlass
              size={"100%"}
              color={isFocused || searchValue.length > 0 ? colors.blue9 : "#959AAB"}
              weight="bold"
            />
          </IconWrapper>
        </SearchInputWrapper>
